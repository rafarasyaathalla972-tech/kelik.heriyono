...
// In-memory cache fallback to guarantee 100% uptime on Vercel
let memoryReports: any[] = [...INITIAL_REPORTS];
...
function ensureDataFile() {
  try {
    ...
    // If filesystem write fails on serverless, fallback to in-memory silently
    console.warn('Filesystem access fallback to memory:', err);
  }
}
...
function readReports(): any[] {
  ...
  try {
    ...
    console.warn('Using in-memory cache for reports:', err);
  }
...
function writeReports(reports: any[]): boolean {
  ...
  try {
    ...
    console.warn('Stored in memory only (filesystem write skipped):', err);
    return true;
  }
}

// ----------------------------------------------------
// REST Endpoints (Accepting both /api/... and /... paths)
// ----------------------------------------------------
...
app.get(['/api/health', '/health'], (req, res) => {
  ...
  res.json({
    status: 'ok',
    platform: process.env.VERCEL ? 'vercel_serverless' : 'cloud_run',
...
// 4. Update report
app.put(['/api/reports/:id', '/reports/:id'], (req, res) => {
  try {
...
// 6. Bulk sync
app.post(['/api/reports/bulk-sync', '/reports/bulk-sync'], (req, res) => {
  try {
